@extends('layouts.admin')

@section('content')

<div class="row">
    <div class="col-md-12">

        @if(session('message'))
            <div class="alert alert-success">{{session('message')}}</div>
        @endif

        <div class="card">
            <div class="card-header">
                <h4>Payments List
                    <a href="{{ url('admin\payment\create')}}" class="btn btn-primary text-white float-end">Add Payments</a>
                </h4>
            </div>
            <div class="card-body">
                <table class="table table-boreder tabel-striped">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Name</th>
                            <th>Image</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($payments as $payment)
                        <tr>
                            <td>{{ $payment->id }}</td>
                            <td>{{ $payment->name }}</td>
                            <td>
                                <img src="/uploads/payment/{{ $payment->image }}" style="width:150px; height:100px;">
                            </td>
                            <td>
                                <a href="{{ url('admin/payment/'.$payment->id.'/edit') }}" class="btn btn-success">Edit</a>
                            </td>
                            <td>
                                    <form method="post" action="{{ url('admin/payment/'.$payment->id) }}">
                                        @method('delete')
                                        @csrf
                                        <button type="submit" class="btn btn-danger">Delete</button>
                                    </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7">No Properties Available</td>
                        </tr>
                    @endforelse
                    <tbody>
                </table>
            </div>
        </div>
    </div>
</div>

@endsection
